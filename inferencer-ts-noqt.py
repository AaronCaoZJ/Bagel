# gemini gen py file, ok for no quantization inferencing
# 显式使用 _move_to_device，类似于 kokoro 版 inferencer 中 
...
class InterleaveInferencer:
    ...
    def _move_to_device(self, inputs: Dict[str, Any]):
        """Helper to move all tensors in a dictionary to the model's device."""
    ...
    def gen_image(
        ...
        **kargs,
        ...
        # ... (rest of the function remains the same, just keeping the structure)
        
        if enable_taylorseer and not enable_speca:
            assert 'taylor_first_enhance' in kargs
            taylor_first_enhance = kargs.get("taylor_first_enhance", 5)
            assert 'taylor_max_order' in kargs
            taylor_max_order = kargs.get("taylor_max_order", 5)
            assert 'taylor_fresh_threshold' in kargs

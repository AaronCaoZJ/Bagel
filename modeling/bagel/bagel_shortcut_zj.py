...
class Bagel(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, config: BagelConfig):
        ...
        if config.visual_gen:
            ...
            # 使用TimestepEmbedder对delta_t进行嵌入
            self.delta_t_embedder = TimestepEmbedder(self.hidden_size)
            self.vae2llm = nn.Linear(self.patch_latent_dim, self.hidden_size)
            self.llm2vae = nn.Linear(self.hidden_size, self.patch_latent_dim)
            self.latent_pos_embed = PositionEmbedding(self.max_latent_size, self.hidden_size)
            self.latent_fusion_net = nn.Sequential(
    ...
    def forward(
        ...
        # shortcut_delta
        packed_delta_t: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
        Args:
        ...
        if self.config.visual_gen:
            ...
            mse = None
            shortcut_mask = (packed_delta_t is not None) & (packed_delta_t > 0) & (packed_timesteps < 1.0)
            flow_mask = (packed_delta_t is not None) & (packed_delta_t == 0)

            packed_latent = torch.zeros_like(packed_latent_clean)
            # packed_timestep_embeds = self.time_embedder(packed_timesteps)

            if flow_mask.any():
                idx = flow_mask.nonzero(as_tuple=True)[0]
                t = packed_timesteps[idx]
                dt = packed_delta_t[idx]
                latent_clean = packed_latent_clean[idx]
                noise_ = noise[idx]
                x_t = (1 - ALPHA * t[:, None]) * latent_clean + t[:, None] * noise_
                latent_t = self.vae2llm(x_t) + self.time_embedder(t) + latent_token_pos_emb[packed_vae_token_indexes[idx]] + self.delta_t_embedder(dt)
                packed_latent[idx] = latent_t
                packed_sequence[packed_vae_token_indexes[idx]] = latent_t

            if shortcut_mask.any():
                idx = shortcut_mask.nonzero(as_tuple=True)[0]
                t = packed_timesteps[idx]
                dt = packed_delta_t[idx]
                latent_clean = packed_latent_clean[idx]
                noise = noise[idx]
                ...
                t2 = t + dt / 2.0
                # t3 = t + dt
                x_t1 = (1 - ALPHA * t1[:, None]) * latent_clean + t1[:, None] * noise
                x_t2 = (1 - ALPHA * t2[:, None]) * latent_clean + t2[:, None] * noise
                # x_t3 = (1 - ALPHA * t3[:, None]) * latent_clean + t3[:, None] * noise
                latent_t1 = self.vae2llm(x_t1) + self.time_embedder(t1) + latent_token_pos_emb[packed_vae_token_indexes[idx]] + self.delta_t_embedder(dt)
                latent_t2 = self.vae2llm(x_t2) + self.time_embedder(t2) + latent_token_pos_emb[packed_vae_token_indexes[idx]] + self.delta_t_embedder(dt)

                latent_t = self.latent_fusion_net(torch.cat([latent_t1, latent_t2], dim=-1))
                packed_latent[idx] = latent_t
                packed_sequence[packed_vae_token_indexes[idx]] = latent_t
    ...
    def _forward_flow(
        ...
        # shortcut_delta
        delta_t: Optional[torch.FloatTensor] = None,
    ):
        ...
        if delta_t is not None:
            dt_embed = self.delta_embedder(delta_t.float().unsqueeze(-1))
            x_t = x_t + dt_embed
